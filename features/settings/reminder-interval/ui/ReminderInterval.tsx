...
type TDataType = Array<{
  title: string;
  value: ISettingsState["reminders"]["interval"];
}>;

export const SetReminderInterval = () => {
  const { setRemindersInterval } = useSettingsActions();
  const { interval } = useReminderSettings();
  ...
  const onPress = (value: ISettingsState["reminders"]["interval"]) => {
